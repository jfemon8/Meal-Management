...
import type { User, Breakfast, BreakfastParticipant } from '../../types';

// ============================================
// Types
// ============================================

type CostMode = 'equal' | 'individual';
...
interface FormData {
  ...
  description: string;
  participants: string[];
  individualCosts: Record<string, string>;
}
...
const BreakfastManagement: React.FC = () => {
  ...
  // Reverse modal state
  const [reverseModal, setReverseModal] = useState<ReverseModalState>({
    open: false,
    breakfast: null,
  });
  ...
  const [deductConfirm, setDeductConfirm] = useState<ConfirmState<Breakfast>>({
    isOpen: false,
    breakfast: null,
    isLoading: false,
  });
  const [deleteConfirm, setDeleteConfirm] = useState<ConfirmState<Breakfast>>({
    isOpen: false,
    breakfast: null,
    isLoading: false,
  });
  ...
  const handleParticipantToggle = (userId: string): void => {
    setFormData((prev) => {
  ...
  const handleIndividualCostChange = (userId: string, cost: string): void => {
    setFormData((prev) => ({
      ...prev,
      individualCosts: {
  ...
  const selectAllParticipants = (): void => {
    ...
    const allIndividualCosts: Record<string, string> = {};
    users.forEach((u) => {
  ...
  const calculateTotalFromIndividual = (): number => {
    return Object.values(formData.individualCosts).reduce(
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (costMode === 'individual') {
      const hasAnyCost = Object.values(formData.individualCosts).some(
        (c) => parseFloat(c) > 0
      );
      if (!hasAnyCost) {
    ...
    try {
      if (costMode === 'equal') {
        ...
        formData.participants
          ...
          .forEach((userId) => {
            participantCosts[userId] = parseFloat(formData.individualCosts[userId]);
          });
  ...
  const openDeductConfirm = (breakfast: Breakfast): void => {
  ...
  const handleDeduct = async (): Promise<void> => {
    ...
    try {
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'খরচ কাটতে সমস্যা হয়েছে');
      setDeductConfirm((prev) => ({ ...prev, isLoading: false }));
    }
  };

  const openDeleteConfirm = (breakfast: Breakfast): void => {
  ...
  // Edit functionality
  const openEditModal = (breakfast: Breakfast): void => {
    const hasVariedCosts = breakfast.participants.some(
      (p, i, arr) => i > 0 && Math.abs(p.cost - arr[0].cost) > 0.01
    );

    const individualCosts: Record<string, string> = {};
    const participants: string[] = [];
    breakfast.participants.forEach((p) => {
      const userId =
        typeof p.user === 'string' ? p.user : (p.user as User)?._id;
      if (userId) {
  ...
  const handleEditParticipantToggle = (userId: string): void => {
    setEditData((prev) => {
      const isSelected = prev.participants.includes(userId);
      const newParticipants = isSelected
        ? prev.participants.filter((id) => id !== userId)
  ...
  const handleEditIndividualCostChange = (userId: string, cost: string): void => {
    setEditData((prev) => ({
      ...prev,
      individualCosts: {
        ...prev.individualCosts,
    ...
    try {
      if (editData.costMode === 'equal') {
        ...
        editData.participants
          ...
          .forEach((userId) => {
            participantCosts[userId] = parseFloat(editData.individualCosts[userId]);
          });
  ...
  const openReverseModal = (breakfast: Breakfast): void => {
    setReverseReason('');
  ...
  const handleReverse = async (): Promise<void> => {
    ...
    try {
      await breakfastService.reverseBreakfast(
        reverseModal.breakfast!._id,
        reverseReason
      );
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'রিভার্স করতে সমস্যা হয়েছে');
    } finally {
  ...
  const hasVariedCosts = (breakfast: Breakfast): boolean => {
    if (breakfast.participants.length <= 1) return false;
    return breakfast.participants.some(
      (p, i, arr) => i > 0 && Math.abs(p.cost - arr[0].cost) > 0.01
    );
  };

  const getParticipantUserId = (participant: BreakfastParticipant): string => {
    return typeof participant.user === 'string'
      ? participant.user
      : (participant.user as User)?._id || '';
  };

  const getParticipantUserName = (participant: BreakfastParticipant): string => {
    return typeof participant.user === 'string'
      ? 'User'
      : (participant.user as User)?.name || 'User';
  };
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="label mb-0">
                  অংশগ্রহণকারী ({formData.participants.length} জন)
                </label>
                <div className="flex gap-2">
                  <button
                    type="button"
                      ...
                      {formData.participants.includes(user._id) && (
                        <div className="relative w-24">
                          ...
                          <input
                            ...
                            onChange={(e: ChangeEvent<HTMLInputElement>) =>
                              handleIndividualCostChange(user._id, e.target.value)
                            }
              ...
              <div className="p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                ...
                {costMode === 'individual' && (
                  <p className="text-sm text-gray-700 dark:text-gray-300">
                    মোট খরচ:{' '}
                    <span className="font-bold">
                      ৳{calculateTotalFromIndividual().toFixed(2)}
                    </span>
                    <span className="text-xs text-gray-500 ml-2">
                      (
                      {
                        formData.participants.filter(
                          (id) => parseFloat(formData.individualCosts[id]) > 0
                        ).length
                      }{' '}
                      জন)
                    </span>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {breakfast.description || 'বিবরণ নেই'}
                    </p>
                    ...
                    {hasVariedCosts(breakfast) && (
                      <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        {breakfast.participants.slice(0, 3).map((p) => (
                          <span key={getParticipantUserId(p)} className="mr-2">
                            {getParticipantUserName(p)}: ৳{p.cost}
                          </span>
                        ))}
                  ...
                  <div className="text-right">
                    ...
                    {!hasVariedCosts(breakfast) && (
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        জনপ্রতি: ৳
                        {(
                          breakfast.totalCost / breakfast.participants.length
                        ).toFixed(2)}
                      </p>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setEditData({ ...editData, costMode: 'equal' })
                    }
                    className={`px-3 py-1 rounded text-sm transition-colors ${
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setEditData({ ...editData, costMode: 'individual' })
                    }
                    className={`px-3 py-1 rounded text-sm transition-colors ${
                      editData.costMode === 'individual'
                        ? 'bg-primary-600 text-white'
                        : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300'
                    }`}
                  >
              ...
              {editData.costMode === 'equal' && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setEditData({ ...editData, totalCost: e.target.value })
                    }
                    className="input"
                    min="0"
                  />
                      ...
                      >
                        ...
                        {editData.participants.includes(user._id) && (
                          <input
                            ...
                            onChange={(e: ChangeEvent<HTMLInputElement>) =>
                              handleEditIndividualCostChange(
                                user._id,
                                e.target.value
                              )
                            }
          ...
          <div className="space-y-2">
            ...
            {deductConfirm.breakfast && (
              <div className="p-3 bg-gray-100 dark:bg-gray-700 rounded-lg text-sm">
                ...
                <p>
                  <strong>অংশগ্রহণকারী:</strong>{' '}
                  {deductConfirm.breakfast.participants?.length} জন
                </p>
              </div>
            )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="p-4 space-y-4">
              <div className="p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg text-sm text-yellow-800 dark:text-yellow-200">
                ...
                <p>
                  এই অপারেশন সকল অংশগ্রহণকারীর ব্যালেন্সে টাকা ফেরত দেবে।
                </p>
                <p className="mt-2">
                  মোট রিফান্ড:{' '}
                  <span className="font-bold">
                    ৳{reverseModal.breakfast?.totalCost}
                  </span>
                  <span className="text-xs ml-1">
                    ({reverseModal.breakfast?.participants.length} জন)
                  </span>
                </p>
              </div>
              ...
              <div>
                <label className="label">
                  রিভার্সের কারণ <span className="text-red-500">*</span>
                </label>
                <textarea
                  value={reverseReason}
