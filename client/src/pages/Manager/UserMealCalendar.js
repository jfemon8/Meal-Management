...
import { mealService, userService, holidayService, monthSettingsService } from '../../services/mealService';
import toast from 'react-hot-toast';
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, isFriday, isSaturday, addDays } from 'date-fns';
import { bn } from 'date-fns/locale';
import { FiChevronLeft, FiChevronRight, FiCheck, FiX, FiCalendar, FiUser, FiRefreshCw } from 'react-icons/fi';

const UserMealCalendar = () => {
    ...
    // Load meal data when user or month changes
    useEffect(() => {
        if (selectedUserId) {
            loadMonthData();
        }
    }, [selectedUserId, currentMonth, mealType]);
    ...
    const loadUsers = async () => {
        ...
        try {
            ...
            const activeUsers = response.filter(u => u.isActive);
            setUsers(activeUsers);
            if (activeUsers.length > 0 && !selectedUserId) {
                setSelectedUserId(activeUsers[0]._id);
            }
        } catch (error) {
    ...
    const handleToggleMeal = async (date, currentStatus) => {
        setUpdating(date);
        try {
            await mealService.toggleMeal(date, !currentStatus, selectedUserId, 1, mealType);
    ...
    const handleBulkToggle = async () => {
        ...
        setBulkLoading(true);
        try {
            const result = await mealService.bulkToggle(bulkStartDate, bulkEndDate, bulkAction === 'on', mealType, selectedUserId);
            toast.success(result.message);
            setShowBulkToggle(false);
            loadMonthData();
        } catch (error) {
            toast.error(error.response?.data?.message || 'বাল্ক আপডেট করতে সমস্যা হয়েছে');
        } finally {
    ...
    const handleRecalculate = async () => {
        ...
        try {
            const year = currentMonth.getFullYear();
            const month = currentMonth.getMonth() + 1;
            const result = await mealService.recalculateMeals(year, month, mealType, selectedUserId);
            toast.success(result.message);
            loadMonthData();
        } catch (error) {
            toast.error(error.response?.data?.message || 'রিক্যালকুলেট করতে সমস্যা হয়েছে');
        } finally {
    ...
    const isCurrentMonthEditable = () => {
        ...
        return currentMonth.getMonth() === thisMonth && currentMonth.getFullYear() === thisYear;
    };
    ...
    // Calculate summary
    const totalMealsOn = mealStatus.filter(s => s.isOn).length;
    const totalMealsOff = mealStatus.filter(s => !s.isOn).length;
    const totalMealCount = mealStatus.reduce((sum, s) => sum + (s.isOn ? s.count : 0), 0);
    const manuallySetCount = mealStatus.filter(s => s.isManuallySet).length;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">ইউজার মিল ক্যালেন্ডার</h1>
                <div className="flex items-center gap-2">
                    <button
                        onClick={handleRecalculate}
                        disabled={recalculating || !isCurrentMonthEditable()}
                        ...
                        title="ম্যানুয়াল ছাড়া মিল রিক্যালকুলেট করুন"
                    >
                        <FiRefreshCw className={`w-4 h-4 ${recalculating ? 'animate-spin' : ''}`} />
                        রিক্যালকুলেট
                    </button>
                    <button
            ...
            <div className="card">
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                        ...
                        <span className="font-medium">ইউজার সিলেক্ট করুন:</span>
                    </div>
                    <select
                        value={selectedUserId}
                        onChange={(e) => setSelectedUserId(e.target.value)}
                        className="flex-1 max-w-md px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:text-white"
                    >
                        {users.map(user => (
                    ...
                    {selectedUser && (
                        <div className="text-sm text-gray-500 dark:text-gray-400">
                            রোল: <span className="font-medium capitalize">{selectedUser.role}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            {showBulkToggle && isCurrentMonthEditable() && (
                <div className="card bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                    ...
                    <p className="mt-3 text-sm text-blue-600 dark:text-blue-400">
                        * সর্বোচ্চ ৩১ দিনের রেঞ্জ সিলেক্ট করতে পারবেন। ম্যানেজার হিসেবে বর্তমান মাসের সব তারিখ পরিবর্তন করা যাবে।
                    </p>
                </div>
            )}
                ...
                <div className="card">
                        ...
                        <div className="text-center">
                            ...
                            {!isCurrentMonthEditable() && (
                                <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
                                    শুধুমাত্র বর্তমান মাস এডিট করা যাবে
                                </p>
                            )}
                        </div>
                        <button
                    ...
                    <div className="mb-4 grid grid-cols-2 md:grid-cols-5 gap-3">
                        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg text-center">
                            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{totalMealsOn}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">মিল অন</p>
                        </div>
                        <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg text-center">
                            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{totalMealsOff}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">মিল অফ</p>
                        </div>
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
                            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{totalMealCount}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">মোট মিল</p>
                        </div>
                        <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg text-center">
                            <p className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{manuallySetCount}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">ম্যানুয়াল</p>
                        </div>
                        <div className="p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg text-center">
                            <p className="text-2xl font-bold text-primary-600 dark:text-primary-400">৳{totalMealCount * currentRate}</p>
                            <p className="text-xs text-gray-600 dark:text-gray-400">আনুমানিক খরচ</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        ...
                        {days.map(day => {
                            ...
                            return (
                                <button
                                    key={day.toISOString()}
                                    onClick={() => canToggle && handleToggleMeal(format(day, 'yyyy-MM-dd'), isOn)}
                                ...
                                >
                                    <div className="flex flex-col items-center justify-center h-full">
                                        ...
                                        {isManuallySet && (
                                            <span className="absolute bottom-0 left-0 w-2 h-2 bg-yellow-500 rounded-full" title="ম্যানুয়ালি সেট করা" />
                                        )}
                                    </div>
                                </button>
                            );
                    ...
                    <div className="mt-6 flex flex-wrap gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="dark:text-gray-300">ম্যানুয়াল ওভাররাইড</span>
                        </div>
                    </div>
                </div>
            )}
...
export default UserMealCalendar;
