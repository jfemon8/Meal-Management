...
const BreakfastReport: React.FC = () => {
  ...
  const getUserSummary = (): UserSummary[] => {
    ...
    breakfasts.forEach((breakfast) => {
      ...
      breakfast.participants.forEach((p: BreakfastParticipant) => {
        const userId =
    ...
    return Object.values(summary)
      .filter((u) => u.participationCount > 0)
  ...
  const getTotals = (): Totals => {
    ...
    breakfasts.forEach((b) => {
      ...
      if (b.isFinalized) {
        finalizedCount++;
        b.participants.forEach((p: BreakfastParticipant) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="card bg-green-50 dark:bg-green-900/20 text-center">
            ...
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">
              ৳{totals.totalDeducted.toFixed(0)}
            </p>
          ...
          <div className="card bg-purple-50 dark:bg-purple-900/20 text-center">
            <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
              {breakfasts.filter((b) => !b.isReversed).length}
            </p>
        ...
        {viewMode === 'daily' && (
          <div className="card">
            <h3 className="font-semibold text-lg mb-4 dark:text-gray-100">
              তারিখ অনুযায়ী বিবরণ
            </h3>
            {breakfasts.length === 0 ? (
              <p className="text-gray-500 dark:text-gray-400 text-center py-8">
                এই মাসে কোনো নাস্তার এন্ট্রি নেই
              </p>
            ) : (
                ...
                <table className="w-full text-sm">
                    ...
                    {breakfasts
                      ...
                      .map((b, index) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-4 text-right font-medium dark:text-gray-200">
                            ৳{b.totalCost.toFixed(0)}
                          </td>
                          <td className="py-3 px-4 text-center dark:text-gray-300">
                            {b.participants.length} জন
                          </td>
                  ...
                  <tfoot>
                    <tr className="border-t-2 font-bold bg-gray-100 dark:bg-gray-700">
                      <td className="py-3 px-4 dark:text-gray-200" colSpan={2}>
                        মোট
                      </td>
                  ...
                  <tbody>
                    {userSummary.map((user, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-center dark:text-gray-300">
                          {user.participationCount} দিন
                        </td>
                        <td className="py-3 px-4 text-right font-medium dark:text-gray-200">
                          ৳{user.totalCost.toFixed(0)}
                        </td>
                        <td className="py-3 px-4 text-right text-green-600 dark:text-green-400">
                          ৳{user.deductedAmount.toFixed(0)}
                        </td>
                        <td className="py-3 px-4 text-right">
                          {user.pendingAmount > 0 ? (
                            <span className="text-yellow-600 dark:text-yellow-400">
                              ৳{user.pendingAmount.toFixed(0)}
                            </span>
                          ) : (
