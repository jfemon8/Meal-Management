...
import type { MonthSettings as MonthSettingsType } from '../../types';
...
interface ExtendedMonthSettings extends MonthSettingsType {
  isDefault?: boolean;
}
...
const MonthSettings: React.FC = () => {
  ...
  const loadSettings = async (): Promise<void> => {
    ...
    try {
      ...
      const settingsData: ExtendedMonthSettings | null = Array.isArray(response)
        ? (response.length > 0 ? response[0] as ExtendedMonthSettings : null)
        : response as ExtendedMonthSettings;
  ...
  const openFinalizeConfirm = (): void => {
    if (!settings?._id) {
  ...
  const handleFinalize = async (): Promise<void> => {
    ...
    try {
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'ফাইনালাইজ করতে সমস্যা হয়েছে');
  ...
  const handlePreview = async (): Promise<void> => {
    ...
    try {
      ...
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || 'প্রিভিউ লোড করতে সমস্যা হয়েছে');
    ...
    if (status === 'due') {
      return (
        <span className="text-red-600 dark:text-red-400 font-medium">
          ৳{Math.abs(amount).toFixed(0)} বাকি
        </span>
      );
    } else if (status === 'advance') {
      return (
        <span className="text-green-600 dark:text-green-400 font-medium">
          ৳{Math.abs(amount).toFixed(0)} অগ্রিম
        </span>
      );
    }
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, lunchRate: e.target.value })
                  }
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, dinnerRate: e.target.value })
                  }
                  className="input pl-10"
                  placeholder="0"
                  min="0"
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg text-center">
                        <p className="text-2xl font-bold text-primary-600">
                          {previewData.grandTotals.totalUsers}
                        </p>
                      ...
                      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg text-center">
                        <p className="text-2xl font-bold text-green-600">
                          {previewData.grandTotals.lunch.totalMeals +
                            previewData.grandTotals.dinner.totalMeals}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          মোট মিল
                        </p>
                      ...
                      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg text-center">
                        <p className="text-2xl font-bold text-blue-600">
                          ৳{previewData.grandTotals.overall.totalCharge.toFixed(0)}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          মোট চার্জ
                        </p>
                      ...
                      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg text-center">
                        <p
                          className={`text-2xl font-bold ${
                            previewData.grandTotals.overall.totalDue > 0
                              ? 'text-red-600'
                              : 'text-green-600'
                          }`}
                        >
                          ৳
                          {Math.abs(
                            previewData.grandTotals.overall.totalDue
                          ).toFixed(0)}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {previewData.grandTotals.overall.totalDue > 0
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-100 dark:bg-gray-700">
                          <tr>
                            <th className="text-left p-2 dark:text-gray-200">
                              নাম
                            </th>
                            <th className="text-center p-2 dark:text-gray-200">
                              দুপুর
                            </th>
                            <th className="text-center p-2 dark:text-gray-200">
                              রাত
                  ...
                  <div className="p-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                    <div className="flex justify-between items-center">
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        রেট: দুপুর ৳{previewData.settings.lunchRate}/মিল, রাত ৳
                        {previewData.settings.dinnerRate}/মিল
                      </p>
                      <button
                        onClick={() => setShowPreview(false)}
