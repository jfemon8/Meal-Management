...
import { FiChevronLeft, FiChevronRight, FiPrinter, FiDownload, FiUsers, FiCheck, FiX } from 'react-icons/fi';
...
const DailyMealReport = () => {
    ...
    const getMealTypeBn = () => mealType === 'lunch' ? 'দুপুর' : 'রাত';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between no-print">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">দৈনিক মিল কাউন্ট</h1>
                ...
                <div ref={printRef} className="space-y-6 print:p-4">
                    ...
                    <div className="hidden print:block text-center mb-8">
                        ...
                        <h2 className="text-xl mt-2">
                            দৈনিক {getMealTypeBn()} মিল রিপোর্ট - {format(selectedDate, 'dd MMMM yyyy', { locale: bn })}
                        </h2>
                    </div>

                    {/* Status Banner */}
                    {(report.isHoliday || report.isDefaultOff) && (
                        <div className={`card ${report.isHoliday ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800'} border`}>
                            <p className={`font-medium ${report.isHoliday ? 'text-red-700 dark:text-red-400' : 'text-yellow-700 dark:text-yellow-400'}`}>
                                {report.isHoliday ? `ছুটির দিন: ${report.holidayName}` : 'ডিফল্ট অফ দিন (শুক্রবার/বিজোড় শনিবার)'}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="card bg-green-50 dark:bg-green-900/20 text-center">
                            ...
                            <p className="text-3xl font-bold text-green-600 dark:text-green-400">{report.summary.mealsOn}</p>
                        ...
                        <div className="card bg-red-50 dark:bg-red-900/20 text-center">
                            ...
                            <p className="text-3xl font-bold text-red-600 dark:text-red-400">{report.summary.mealsOff}</p>
                        ...
                        <div className="card bg-purple-50 dark:bg-purple-900/20 text-center">
                            <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{report.summary.totalMealCount}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400">মোট মিল সংখ্যা</p>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-gray-50 dark:bg-gray-700">
                                        ...
                                        <th className="text-center py-3 px-4 dark:text-gray-200">সংখ্যা</th>
                                        <th className="text-center py-3 px-4 dark:text-gray-200">ম্যানুয়াল</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr key={user.user._id} className={`border-b dark:border-gray-700 ${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-750'}`}>
                                            ...
                                            <td className="py-3 px-4 text-center">
                                                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                                                    ...
                                                    {user.isOn ? <FiCheck className="w-3 h-3" /> : <FiX className="w-3 h-3" />}
                                                    {user.isOn ? 'অন' : 'অফ'}
                                                </span>
                                            </td>
                                            <td className="py-3 px-4 text-center font-medium dark:text-gray-200">
                                                {user.count > 1 ? (
                                                    <span className="text-purple-600 dark:text-purple-400">{user.count} (গেস্ট)</span>
                                                ) : (
                                                    user.count
                                                )}
                                            </td>
                                            <td className="py-3 px-4 text-center">
                                                {user.isManuallySet ? (
                                                    <span className="text-xs text-blue-600 dark:text-blue-400">হ্যাঁ</span>
                                                ) : (
                                ...
                                <tfoot>
                                    <tr className="border-t-2 font-bold bg-gray-100 dark:bg-gray-700">
                                        ...
                                        <td className="py-3 px-4 text-center text-purple-600 dark:text-purple-400">{report.summary.totalMealCount}</td>
