...
type FilterStatus = 'all' | 'due' | 'advance' | 'settled';
...
const LunchStatement: React.FC = () => {
  ...
  const { data: lunchSummary, isLoading: summaryLoading } = useGroupLunchSummary(
    selectedYear,
    selectedMonth
  ) as { data: LunchSummary | undefined; isLoading: boolean };
  ...
  const filteredMembers = useMemo<LunchMember[]>(() => {
    ...
    return lunchSummary.members.filter((member) => {
      ...
      // Status filter
      const matchesStatus =
        filterStatus === 'all' ? true : member.dueAdvance.type === filterStatus;
  ...
  const handleDownloadGroupPDF = async (): Promise<void> => {
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {myGroup ? `${myGroup.name} গ্রুপ` : 'গ্রুপ'} এর সদস্যদের লাঞ্চ
                ...
                {filteredMembers.map((member) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      >
                        {member.dueAdvance.type === 'due' &&
                          `${formatCurrency(member.dueAdvance.amount)} বকেয়া`}
                        {member.dueAdvance.type === 'advance' &&
                          `${formatCurrency(member.dueAdvance.amount)} অগ্রিম`}
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        onClick={() =>
                          handleDownloadUserPDF(member._id, member.name)
                        }
      ...
      {filteredMembers.length !== lunchSummary?.members?.length && (
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
          মোট {lunchSummary?.members?.length} সদস্যের মধ্যে{' '}
          {filteredMembers.length} জন দেখানো হচ্ছে
        </p>
      )}
    </div>
  );
