...
import {
    FiUsers, FiUser, FiEdit2, FiKey, FiToggleLeft, FiToggleRight,
    FiSearch, FiChevronDown, FiChevronRight, FiFilter, FiRefreshCw,
    FiShield, FiUserCheck, FiUserX, FiX, FiCheck, FiMail, FiPhone,
    FiDollarSign, FiTrash2, FiAlertTriangle
} from 'react-icons/fi';
import BDTIcon from '../../components/Icons/BDTIcon';

const GroupwiseAdmin = () => {
    ...
    const loadGroups = async () => {
        ...
        try {
            ...
            let filteredByRole = response;
            if (isOnlyGroupManager && user?.group) {
                // Group managers only see their own group
                filteredByRole = response.filter(g => g._id === user.group);
            }
    ...
    const handleSaveUser = async (e) => {
        ...
        try {
            ...
            if (isAdmin && editFormData.role !== selectedUser.role) {
                await userService.updateUserRole(selectedUser._id, editFormData.role);
            }
    ...
    const handleToggleUserStatus = async (user) => {
        ...
        if (!window.confirm(`আপনি কি নিশ্চিত যে ${user.name} কে ${action} করতে চান?`)) return;

        try {
            await userService.updateUserStatus(user._id, newStatus);
            toast.success(`${user.name} ${action} করা হয়েছে`);

            // Refresh the group members
            if (user.group) {
                await loadGroupMembers(user.group);
            }
            loadGroups();
        } catch (error) {
            toast.error(error.response?.data?.message || 'স্ট্যাটাস পরিবর্তন করতে সমস্যা হয়েছে');
        }
    };

    const handleDeleteUser = async (user) => {
        if (!window.confirm(`আপনি কি নিশ্চিত যে "${user.name}" ডিলিট করতে চান? এটি পূর্বাবস্থায় ফেরানো যাবে না।`)) return;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Groups Summary - Different view for group managers */}
            {isOnlyGroupManager ? (
                <div className="grid grid-cols-3 gap-4">
                    <div className="card text-center">
                        <p className="text-3xl font-bold text-primary-600">{groups.reduce((sum, g) => sum + (g.memberCount || 0), 0)}</p>
                    ...
                    <div className="card text-center">
                        <p className="text-3xl font-bold text-red-600">
                            {Object.values(groupMembers).flat().filter(m => m && !m.isActive).length || '-'}
                        </p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="card text-center">
                        <p className="text-3xl font-bold text-blue-600">{groups.filter(g => g.isActive).length}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">সক্রিয় গ্রুপ</p>
                    </div>
                    <div className="card text-center">
                        <p className="text-3xl font-bold text-green-600">{groups.reduce((sum, g) => sum + (g.memberCount || 0), 0)}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">মোট সদস্য</p>
                    </div>
                    <div className="card text-center">
                        <p className="text-3xl font-bold text-purple-600">{groups.filter(g => g.manager).length}</p>
                    ...
                    <div key={group._id} className="card overflow-hidden">
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${group.isActive ? 'bg-primary-100 dark:bg-primary-900/30' : 'bg-gray-200 dark:bg-gray-700'
                                    }`}>
                                    <FiUsers className={`w-6 h-6 ${group.isActive ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'}`} />
                                </div>
                                <div>
                                    <h3 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                                        {group.name}
                                        {group.code && (
                                            <span className="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded">{group.code}</span>
                                        )}
                                        {!group.isActive && (
                            ...
                            <div className="mt-4 pt-4 border-t dark:border-gray-700">
                                {loadingMembers[group._id] ? (
                                    ...
                                    <p className="text-center text-gray-500 dark:text-gray-400 py-4">
                                        {groupMembers[group._id]?.length === 0 ? 'কোন সদস্য নেই' : 'ফিল্টার অনুযায়ী কোন সদস্য নেই'}
                                    </p>
                                ) : (
                                    <div className="space-y-2">
                                        {getFilteredMembers(groupMembers[group._id]).map(member => (
                                            <div
                                                key={member._id}
                                                className={`flex flex-col lg:flex-row lg:items-center justify-between gap-3 p-3 rounded-lg ${member.isActive
                                                        ? 'bg-gray-50 dark:bg-gray-800/50'
                                                        : 'bg-red-50 dark:bg-red-900/10'
                                                    }`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${member.isActive
                                                            ? 'bg-primary-100 dark:bg-primary-900/30'
                                                            : 'bg-red-100 dark:bg-red-900/30'
                                                        }`}>
                                                        <span className={`font-medium ${member.isActive
                                                    ...
                                                    <div>
                                                        ...
                                                        {member.balances && (
                                                            <div className="flex flex-wrap items-center gap-3 mt-1 text-xs">
                                                                <span className={`flex items-center gap-1 ${member.balances.lunch?.amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                                    <BDTIcon className="w-3 h-3" />
                                                                    লাঞ্চ: {member.balances.lunch?.amount?.toFixed(0) || 0}
                                                                </span>
                                                                <span className={`flex items-center gap-1 ${member.balances.dinner?.amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                                    <BDTIcon className="w-3 h-3" />
                                                                    ডিনার: {member.balances.dinner?.amount?.toFixed(0) || 0}
                                                                </span>
                                                                <span className={`flex items-center gap-1 ${member.balances.breakfast?.amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                                                    <BDTIcon className="w-3 h-3" />
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 ml-13 lg:ml-0">
                                                    {/* Edit - Admin+ or Group Manager with permission */}
                                                    {(isAdmin || (isOnlyGroupManager && group.settings?.canManagerEditUsers)) && (
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                handleEditUser({ ...member, group: group._id, groupSettings: group.settings });
                                                            }}
                                                    ...
                                                    {isAdmin && (
                                                        <button
                                                            ...
                                                            className={`btn btn-sm flex items-center gap-1 ${member.isActive
