...
import { IUser, IUserDocument, IUserModel, BalanceType } from '../types';
...
// Get total balance
userSchema.methods.getTotalBalance = function (): number {
...
userSchema.methods.isBalanceFrozen = function (balanceType: BalanceType): boolean {
...
userSchema.methods.hasLowBalance = function (): boolean {
...
userSchema.methods.getBalanceDetails = function (balanceType: BalanceType) {
    if (!['breakfast', 'lunch', 'dinner'].includes(balanceType)) {
        return null;
    }
...
userSchema.methods.getAllPermissions = function (): string[] {
    const { getRolePermissions } = require('../config/permissions');
    const rolePerms: string[] = getRolePermissions(this.role);
    const customPerms: string[] = this.permissions || [];
