...
import { IPerformanceMetricDocument } from '../types';
...
performanceMetricSchema.statics.getApiSummary = async function(hours: number = 24) {
...
performanceMetricSchema.statics.getSystemMetrics = async function(hours: number = 24, limit: number = 100) {
    const since = new Date(Date.now() - hours * 60 * 60 * 1000);
...
performanceMetricSchema.statics.getSlowEndpoints = async function(threshold: number = 1000, hours: number = 24) {
    const since = new Date(Date.now() - hours * 60 * 60 * 1000);
...
export default mongoose.model<IPerformanceMetricDocument>('PerformanceMetric', performanceMetricSchema);
