...
ruleOverrideSchema.virtual('isExpired').get(function() {
    if (!this.expiresAt) return false;
    return new Date() > this.expiresAt;
});

// Method to check if date falls within this override
ruleOverrideSchema.methods.appliesToDate = function(date) {
...
ruleOverrideSchema.statics.getPriorityForRole = function(role) {
    const priorityMap = {
        'superadmin': 4,
        'admin': 4,
        'manager': 3,
...
module.exports = mongoose.model('RuleOverride', ruleOverrideSchema);
