...
import { IFeatureFlagDocument } from '../types';
...
featureFlagSchema.methods.isActiveForUser = function(user: any, environment: string = 'production'): boolean {
    // Check if globally enabled
    if (!this.isEnabled) return false;

    // Check environment
    if (!(this.environments as any)[environment]) return false;
    ...
    if (this.allowedUsers.length > 0) {
        ...
        if (this.allowedUsers.some((u: any) => u.toString() === userId)) {
            return true;
        }
    }
...
featureFlagSchema.methods.hashUserId = function(userId: string): number {
    let hash = 0;
...
featureFlagSchema.statics.getActiveFeatures = async function(user: any, environment: string = 'production'): Promise<Record<string, any>> {
...
featureFlagSchema.statics.isFeatureEnabled = async function(key: string, user: any, environment: string = 'production'): Promise<boolean> {
...
// Add history entry
featureFlagSchema.methods.addHistory = function(action: string, changedBy: mongoose.Types.ObjectId, previousValue: any, newValue: any, reason: string = ''): void {
    this.history.push({
        action,
        changedBy,
        changedAt: new Date(),
