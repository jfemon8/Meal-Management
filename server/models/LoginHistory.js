...
loginHistorySchema.statics.getRecentAttempts = async function(userId, hours = 24) {
...
loginHistorySchema.statics.getFailedAttemptsCount = async function(userId, minutes = 15) {
...
loginHistorySchema.statics.checkSuspiciousActivity = async function(userId, deviceInfo) {
    ...
    const knownDeviceIds = recentLogins.map(login => login.deviceInfo?.deviceId).filter(Boolean);
    if (deviceInfo.deviceId && !knownDeviceIds.includes(deviceInfo.deviceId)) {
        suspiciousReasons.push('new_device');
    }

    // Check for new IP
    const knownIPs = recentLogins.map(login => login.deviceInfo?.ip).filter(Boolean);
...
loginHistorySchema.statics.getLoginStats = async function(userId, days = 30) {
...
loginHistorySchema.statics.cleanupOldRecords = async function(days = 90) {
