...
import {
    ...
    IHolidayDocument,
    IUserDocument,
    IMonthSettingsDocument,
    IGlobalSettingsDocument,
    MealOffResult,
    CutoffResult,
    MealTogglePermission,
    EffectiveMealStatus,
    IMealDocument
} from '../types';
...
interface MonthFinalizedResult {
    isFinalized: boolean;
    monthSettings: IMonthSettingsDocument | null;
}

interface CurrentMonthResult {
...
interface MealToggleParams {
    ...
    date: Date;
    mealType: MealType;
    settings?: IGlobalSettingsDocument | null;
}

interface EffectiveMealStatusParams {
    date: Date;
    mealType: MealType;
    manualMeal: IMealDocument | null;
    holidays: IHolidayDocument[];
    settings?: IGlobalSettingsDocument | null;
}

interface ValidateMealToggleParams {
    user: IUserDocument;
    date: Date;
    mealType: MealType;
    targetUserId?: string;
...
interface ValidateMealToggleResult {
    valid: boolean;
    error: string | null;
 ...
 */
const isFriday = (date: Date | string): boolean => new Date(date).getDay() === 5;

/**
 * Check if a date is Saturday
 */
const isSaturday = (date: Date | string): boolean => new Date(date).getDay() === 6;
 ...
 */
const isEvenSaturday = (date: Date | string): boolean => {
    const d = new Date(date);
    if (d.getDay() !== 6) return false;
    const dayOfMonth = d.getDate();
    const saturdayNumber = Math.ceil(dayOfMonth / 7);
 ...
 */
const getApplicableHoliday = (date: Date | string, holidays: IHolidayDocument[], holidayPolicy: HolidayPolicy): IHolidayDocument | null => {
 ...
 */
const isDefaultMealOff = async (date: Date | string, holidays: IHolidayDocument[] = [], settings: IGlobalSettingsDocument | null = null): Promise<MealOffResult> => {
    if (!settings) {
        settings = await GlobalSettings.getSettings();
    }

    const weekendPolicy: WeekendPolicy = settings.weekendPolicy || {};
    const holidayPolicy: HolidayPolicy = settings.holidayPolicy || {};
    const d = new Date(date);
 ...
 */
const isCutoffTimePassed = async (mealType: MealType, targetDate: Date | string, settings: IGlobalSettingsDocument | null = null): Promise<CutoffResult> => {
    if (!settings) {
        settings = await GlobalSettings.getSettings();
    }

    const cutoffTimes = settings.cutoffTimes || { lunch: 10, dinner: 16 };
    const cutoffHour: number = cutoffTimes[mealType] || (mealType === 'lunch' ? 10 : 16);
 ...
 */
const isMonthFinalized = async (date: Date | string): Promise<MonthFinalizedResult> => {
    const d = new Date(date);
 ...
 */
const isWithinCurrentMonth = async (date: Date | string): Promise<CurrentMonthResult> => {
    const now = new Date();
 ...
 */
const getMealTogglePermission = async ({ user, date, mealType, settings = null }: MealToggleParams): Promise<MealTogglePermission> => {
    if (!settings) {
        settings = await GlobalSettings.getSettings();
    }
 ...
 */
const getEffectiveMealStatus = async ({ date, mealType, manualMeal, holidays, settings = null }: EffectiveMealStatusParams): Promise<EffectiveMealStatus> => {
    if (!settings) {
        settings = await GlobalSettings.getSettings();
    }
    ...
    const isDefaultOn: boolean = defaultStatus[mealType] === true;
 ...
 */
const getApplicableHolidays = async (startDate: Date, endDate: Date, settings: IGlobalSettingsDocument | null = null): Promise<IHolidayDocument[]> => {
    if (!settings) {
        settings = await GlobalSettings.getSettings();
    }
    ...
    // Get all holidays
    const allHolidays: IHolidayDocument[] = await Holiday.find({
        date: { $gte: startDate, $lte: endDate },
        isActive: true
    });

    // Filter based on policy
    return allHolidays.filter((h: IHolidayDocument) => {
 ...
 */
const validateMealToggle = async ({ user, date, mealType, targetUserId }: ValidateMealToggleParams): Promise<ValidateMealToggleResult> => {
    ...
    if (targetUserId && targetUserId.toString() !== (user as any)._id.toString()) {
        const isManager = ['manager', 'admin', 'superadmin'].includes(user.role);
        if (!isManager) {
            return {
                valid: false,
