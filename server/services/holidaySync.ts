...
const NAGER_API_BASE: string = 'https://date.nager.at/api/v3';
 ...
 */
const fetchBangladeshHolidays = async (year: number): Promise<NagerApiHoliday[]> => {
    try {
        const response: AxiosResponse<NagerApiHoliday[]> = await axios.get(`${NAGER_API_BASE}/PublicHolidays/${year}/BD`, {
 ...
 */
const mapHolidayType = (types: string[] = []): string => {
 ...
 */
const syncHolidays = async (year: number, syncedBy: Types.ObjectId | null = null): Promise<SyncResult> => {
    ...
    try {
        ...
        for (const apiHoliday of apiHolidays) {
            try {
                ...
                const existing: any = await Holiday.findOne({
                    date: {
...
const checkApiAvailability = async (): Promise<boolean> => {
    try {
        const response: AxiosResponse = await axios.get(`${NAGER_API_BASE}/AvailableCountries`, {
            timeout: 5000
        });
