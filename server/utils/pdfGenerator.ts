...
interface GroupLunchStatementData {
    group: GroupInfo;
    period: PeriodInfo;
    members: MemberInfo[];
    summary: SummaryInfo;
 ...
 */
const generateLunchStatementPDF = (data: LunchStatementData, res: Response): void => {
    ...
    const dailyHeaders: string[] = ['তারিখ', 'দিন', 'স্ট্যাটাস', 'মিল সংখ্যা', 'মন্তব্য'];
    const dailyData: string[][] = [dailyHeaders];

    data.dailyDetails.forEach((day: DailyDetail) => {
 ...
 */
const generateGroupLunchStatementPDF = (data: GroupLunchStatementData, res: Response): void => {
    const doc = new PDFDocument({
        size: 'A4',
        layout: 'landscape',
        margin: 40,
    ...
    // ===== Members Table =====
    const headers: string[] = ['ক্র.', 'নাম', 'মিল সংখ্যা', 'মোট চার্জ', 'ব্যালেন্স', 'বকেয়া/অগ্রিম', 'স্ট্যাটাস'];
    const tableData: string[][] = [headers];

    data.members.forEach((member: MemberInfo, idx: number) => {
 ...
 */
function drawTable(doc: any, data: string[][], x: number, y: number, colWidths: number[], hasHeader: boolean = false): number {
    const rowHeight = 20;
    let currentY = y;
    ...
    data.forEach((row: string[], rowIndex: number) => {
        ...
        if (isHeader) {
            doc.rect(x, currentY, colWidths.reduce((a: number, b: number) => a + b, 0), rowHeight)
                .fill('#e0e0e0');
            doc.fillColor('black');
        }
 ...
 */
function formatDateBn(date: Date | string): string {
    if (!date) return '';
