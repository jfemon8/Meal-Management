...
import { syncHolidays, checkApiAvailability } from '../services/holidaySync';

/**
 * Initialize holiday sync cron jobs
 * Schedules automatic holiday syncing from external API
 */
const initHolidayCron = (): void => {
    // Check if cron is valid before scheduling
    const isValidCron = (expression: string): boolean => cron.validate(expression);
 ...
 * @returns Promise
 */
const runImmediateSync = async (year: number | null = null): Promise<any> => {
