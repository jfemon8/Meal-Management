...
import { formatDateISO, formatDateBn, formatDateTime, nowBD, toBDTime, startOfDayBD, getDefaultMonthRange, isValidDateRange } from '../utils/dateUtils';
...
router.get('/', protect, async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (year && month) {
            let settings: any = await MonthSettings.findOne({
...
router.post('/', protect, isManager, [
    ...
    try {
        ...
        let settings: any = await MonthSettings.findOne({ year, month });
...
// @access  Private (Manager+)
router.put('/:id/finalize', protect, isManager, async (req: AuthRequest, res: Response) => {
    try {
        const settings: any = await MonthSettings.findById(req.params.id);
        if (!settings) {
...
// @access  Private (Manager+)
router.post('/:id/carry-forward', protect, isManager, async (req: AuthRequest, res: Response) => {
    try {
        const settings: any = await MonthSettings.findById(req.params.id);
        if (!settings) {
            return res.status(404).json({ message: 'মাসের সেটিংস পাওয়া যায়নি' });
...
// @access  Private (Manager+)
router.get('/:id/preview', protect, isManager, async (req: AuthRequest, res: Response) => {
    try {
        const settings: any = await MonthSettings.findById(req.params.id);
        if (!settings) {
            return res.status(404).json({ message: 'মাসের সেটিংস পাওয়া যায়নি' });
        ...
        const userPreviews = users.map((user: any) => {
            const userLunchMeals = allLunchMeals.filter((m: any) => m.user.toString() === user._id.toString());
            const userDinnerMeals = allDinnerMeals.filter((m: any) => m.user.toString() === user._id.toString());
            ...
            dates.forEach((date: any) => {
                ...
                // Lunch calculation
                const lunchMeal = userLunchMeals.find((m: any) => formatDate(m.date) === dateStr);
                if (lunchMeal) {
                ...
                // Dinner calculation
                const dinnerMeal = userDinnerMeals.find((m: any) => formatDate(m.date) === dateStr);
                if (dinnerMeal) {
            ...
            let breakfastCost = 0;
            allBreakfasts.forEach((b: any) => {
                const participation = b.participants.find((p: any) => p.user.toString() === user._id.toString());
                if (participation) {
        ...
        const grandTotals = {
            ...
            lunch: {
                totalMeals: userPreviews.reduce((sum: number, u: any) => sum + u.lunch.meals, 0),
                totalCharge: userPreviews.reduce((sum: number, u: any) => sum + u.lunch.charge, 0),
                totalBalance: userPreviews.reduce((sum: number, u: any) => sum + u.lunch.balance, 0),
                totalDue: userPreviews.reduce((sum: number, u: any) => sum + u.lunch.due, 0)
            },
            dinner: {
                totalMeals: userPreviews.reduce((sum: number, u: any) => sum + u.dinner.meals, 0),
                totalCharge: userPreviews.reduce((sum: number, u: any) => sum + u.dinner.charge, 0),
                totalBalance: userPreviews.reduce((sum: number, u: any) => sum + u.dinner.balance, 0),
                totalDue: userPreviews.reduce((sum: number, u: any) => sum + u.dinner.due, 0)
            },
            breakfast: {
                totalCost: userPreviews.reduce((sum: number, u: any) => sum + u.breakfast.cost, 0),
                totalBalance: userPreviews.reduce((sum: number, u: any) => sum + u.breakfast.balance, 0),
                totalDue: userPreviews.reduce((sum: number, u: any) => sum + u.breakfast.due, 0)
            },
            overall: {
                totalCharge: userPreviews.reduce((sum: number, u: any) => sum + u.total.charge, 0),
                totalBalance: userPreviews.reduce((sum: number, u: any) => sum + u.total.balance, 0),
                totalDue: userPreviews.reduce((sum: number, u: any) => sum + u.total.due, 0),
                usersWithDue: userPreviews.filter((u: any) => u.total.due > 0).length,
                usersWithAdvance: userPreviews.filter((u: any) => u.total.due < 0).length,
                usersSettled: userPreviews.filter((u: any) => u.total.due === 0).length
            }
        };
        ...
        res.json({
            ...
            grandTotals,
            userPreviews: userPreviews.sort((a: any, b: any) => b.total.due - a.total.due), // Sort by due (highest first)
            holidays: holidays.map((h: any) => ({ date: formatDate(h.date), name: h.nameBn }))
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'সার্ভার এরর' });
...
// @access  Private
router.get('/current', protect, async (req: AuthRequest, res: Response) => {
    try {
        const today = nowBD();
