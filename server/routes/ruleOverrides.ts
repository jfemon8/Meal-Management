...
// @access  Private
router.get('/check', protect, async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.json({
            ...
            mealType,
            userId: targetUserId,
            applicableOverrides: overrides.map((o: any) => ({
                _id: o._id,
                action: o.action,
                priority: o.priority,
...
// @access  Private
router.get('/effective-status', protect, async (req: AuthRequest, res: Response) => {
    try {
        const { date, userId, mealType = 'lunch' } = req.query;
...
// @access  Private (Creator or Admin+)
router.post('/:id/toggle', protect, isManager, async (req: AuthRequest, res: Response) => {
    try {
        const override = await RuleOverride.findById(req.params.id);
        if (!override) {
            return res.status(404).json({ message: 'রুল ওভাররাইড পাওয়া যায়নি' });
