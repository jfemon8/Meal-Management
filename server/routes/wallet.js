...
router.get('/statement/pdf', protect, async (req, res) => {
    try {
        ...
        doc.text(`Date: ${new Date().toLocaleDateString('bn-BD')}`);
        doc.moveDown();
        ...
        if (transactions.length === 0) {
            ...
            transactions.forEach(txn => {
                ...
                doc.text(new Date(txn.createdAt).toLocaleDateString('en-GB'), x, y, { width: colWidths.date });
                x += colWidths.date;
        ...
        doc.text(`Generated on: ${new Date().toLocaleString('bn-BD')}`, { align: 'center', color: 'gray' });
