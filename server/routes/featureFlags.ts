...
router.get('/active', protect, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const activeFeatures = await (FeatureFlag as any).getActiveFeatures(req.user, environment);
...
// @access  Private
router.get('/check/:key', protect, async (req: AuthRequest, res: Response) => {
    try {
        const environment = process.env.NODE_ENV || 'development';
        const isEnabled = await (FeatureFlag as any).isFeatureEnabled(req.params.key, req.user, environment);
...
// @access  Private (SuperAdmin)
router.put('/:id', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const flag: any = await FeatureFlag.findById(req.params.id);
        if (!flag) {
            return res.status(404).json({ message: 'ফিচার ফ্ল্যাগ পাওয়া যায়নি' });
        }
...
// @access  Private (SuperAdmin)
router.put('/:id/config', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const flag: any = await FeatureFlag.findById(req.params.id);
        if (!flag) {
            return res.status(404).json({ message: 'ফিচার ফ্ল্যাগ পাওয়া যায়নি' });
...
router.put('/:id/beta-users', protect, isSuperAdmin, [
    ...
    try {
        ...
        if (action === 'add') {
            const uniqueUsers = [...new Set([...flag.allowedUsers.map((u: any) => u.toString()), ...userIds])];
            flag.allowedUsers = uniqueUsers;
        } else {
            flag.allowedUsers = flag.allowedUsers.filter((u: any) => !userIds.includes(u.toString()));
        }
...
// @access  Private (Admin+)
router.get('/categories/list', protect, isAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const categories = [
