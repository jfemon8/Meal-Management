...
import { canAccessGroup, getGroupFilteredUsers, checkGroupPermission } from '../middleware/groupAccess';
import { generateLunchStatementPDF, generateGroupLunchStatementPDF } from '../utils/pdfGenerator';
import {
    getDatesBetween,
    formatDate,
    formatDateISO,
...
router.get('/my-group', protect, isManager, canAccessGroup, async (req: AuthRequest, res: Response) => {
    try {
...
// @access  Private (Manager+)
router.get('/members', protect, isManager, canAccessGroup, checkGroupPermission('canManagerViewReports'), async (req: AuthRequest, res: Response) => {
    try {
        const groupId = (req as any).group?._id || req.query.groupId;
...
// @access  Private (Manager+)
router.get('/lunch-summary', protect, isManager, canAccessGroup, checkGroupPermission('canManagerViewReports'), async (req: AuthRequest, res: Response) => {
    try {
        ...
        let userQuery: any = { isActive: true, isDeleted: { $ne: true } };
        if (req.user!.role === 'manager') {
        ...
        const memberReports = users.map((user: any) => {
            const userMeals = allLunchMeals.filter((m: any) => m.user.toString() === user._id.toString());
            let totalMeals = 0;
            let daysOn = 0;
            ...
            dates.forEach((date: any) => {
                ...
                const meal = userMeals.find((m: any) => formatDate(m.date) === dateStr);

                let isOn: boolean, count: number;
                if (meal) {
                    isOn = meal.isOn;
                    count = meal.count;
                } else {
        ...
        // Sort by name
        memberReports.sort((a: any, b: any) => a.name.localeCompare(b.name, 'bn'));

        // Calculate summary
        const summary = {
            totalMembers: memberReports.length,
            totalMeals: memberReports.reduce((sum: number, m: any) => sum + m.totalMeals, 0),
            totalCharge: memberReports.reduce((sum: number, m: any) => sum + m.totalCharge, 0),
            totalDue: memberReports.filter((m: any) => m.dueAdvance.type === 'due').reduce((sum: number, m: any) => sum + m.dueAdvance.amount, 0),
            totalAdvance: memberReports.filter((m: any) => m.dueAdvance.type === 'advance').reduce((sum: number, m: any) => sum + m.dueAdvance.amount, 0),
            membersWithDue: memberReports.filter((m: any) => m.dueAdvance.type === 'due').length,
            membersWithAdvance: memberReports.filter((m: any) => m.dueAdvance.type === 'advance').length
        };

        res.json({
            group: (req as any).group ? { _id: (req as any).group._id, name: (req as any).group.name } : null,
            period: {
                year: parseInt(year as string),
                month: parseInt(month as string),
...
// @access  Private (Manager+)
router.get('/user/:userId/lunch-statement', protect, isManager, canAccessGroup, checkGroupPermission('canManagerViewReports'), async (req: AuthRequest, res: Response) => {
    try {
        const { userId } = req.params;
        const { year, month, startDate: customStart, endDate: customEnd } = req.query;

        // Check if user belongs to manager's group
        const user: any = await User.findById(userId).select('-password').populate('group', 'name');
        if (!user) {
            return res.status(404).json({ message: 'ইউজার পাওয়া যায়নি' });
        }

        // For managers, verify user is in their group
        if (req.user!.role === 'manager' && (req as any).group) {
            if (!user.group || user.group._id.toString() !== (req as any).group._id.toString()) {
                return res.status(403).json({ message: 'এই ইউজার আপনার গ্রুপের সদস্য নয়' });
            }
        }

        // Determine date range
        let startDate: Date, endDate: Date, lunchRate = 0;
        ...
        if (customStart && customEnd) {
            ...
            if (monthSettings) {
                ...
                const range = getDefaultMonthRange(parseInt(year as string), parseInt(month as string));
        ...
        const dailyDetails = dates.map((date: any) => {
            const dateStr = formatDate(date);
            const isOff = isDefaultMealOff(date, holidayDates);
            const holiday = holidays.find((h: any) => formatDate(h.date) === dateStr);
            const meal = lunchMeals.find((m: any) => formatDate(m.date) === dateStr);

            let isOn: boolean, count: number, isManuallySet = false;
            if (meal) {
                isOn = meal.isOn;
                count = meal.count;
                isManuallySet = true;
            ...
            return {
                ...
                isManuallySet,
                isHoliday: !!holiday,
                holidayName: (holiday as any)?.nameBn,
                isWeekend: isOff && !holiday
            };
        });
...
// @access  Private (Manager+)
router.get('/user/:userId/lunch-statement/pdf', protect, isManager, canAccessGroup, checkGroupPermission('canManagerViewReports'), async (req: AuthRequest, res: Response) => {
    try {
        const { userId } = req.params;
        const { year, month, startDate: customStart, endDate: customEnd } = req.query;
...
// @access  Private (Manager+)
router.get('/group-statement/pdf', protect, isManager, canAccessGroup, checkGroupPermission('canManagerViewReports'), async (req: AuthRequest, res: Response) => {
    try {
        const { year, month, groupId: queryGroupId } = req.query;
        const groupId = (req as any).group?._id || queryGroupId;
        ...
        // Sort by name
        members.sort((a: any, b: any) => a.name.localeCompare(b.name, 'bn'));

        // Summary
        const summary = {
            totalMeals: members.reduce((sum: number, m: any) => sum + m.totalMeals, 0),
            totalCharge: members.reduce((sum: number, m: any) => sum + m.totalCharge, 0),
            totalDue: members.filter((m: any) => m.dueAdvance.type === 'due').reduce((sum: number, m: any) => sum + m.dueAdvance.amount, 0)
        };
