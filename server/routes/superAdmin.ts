...
import { protect, isSuperAdmin, isAdmin } from '../middleware/auth';
...
router.put('/bulk-rate-update', protect, isSuperAdmin, [
    ...
    try {
        ...
        updateFields.modifiedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.put('/users/:id/soft-delete', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        ...
        user.deletedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.put('/users/:id/restore', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const user = await User.findById(req.params.id) as any;
        if (!user) {
            return res.status(404).json({ message: 'ইউজার পাওয়া যায়নি' });
        ...
        user.restoredBy = (req.user as any)._id;
        await user.save();
...
// @access  Private (SuperAdmin)
router.get('/users/deleted', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
...
// @access  Private (SuperAdmin)
router.delete('/users/:id/permanent', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
...
router.put('/transactions/:id/correct', protect, isSuperAdmin, [
    ...
    try {
        ...
        const transaction = await Transaction.findById(req.params.id) as any;
        if (!transaction) {
        ...
        transaction.corrections.push({
            ...
            correctedBy: (req.user as any)._id,
            reason,
            originalValues,
        ...
        transaction.lastCorrectedBy = (req.user as any)._id;

        await transaction.save();

        // Recalculate user balance if amount changed
        if (amount !== undefined && amount !== originalValues.amount) {
            const user = await User.findById(transaction.user) as any;
            if (user) {
...
router.put('/month-settings/:id/force-update', protect, isSuperAdmin, [
    ...
    try {
        ...
        const settings = await MonthSettings.findById(req.params.id) as any;
        if (!settings) {
        ...
        settings.modifiedBy = (req.user as any)._id;
        settings.lastForceUpdatedAt = new Date();
        settings.lastForceUpdatedBy = (req.user as any)._id;
...
router.put('/breakfast/:id/force-update', protect, isSuperAdmin, [
    ...
    try {
        ...
        const breakfast = await Breakfast.findById(req.params.id) as any;
        if (!breakfast) {
        ...
        breakfast.lastForceUpdatedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.get('/corrections/history', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const { type, limit = 50, skip = 0 } = req.query as { type?: string; limit?: string; skip?: string };
...
// @access  Private (SuperAdmin)
router.get('/db/stats', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const stats = {
            users: {
...
// @access  Private (SuperAdmin)
router.post('/db/cleanup-orphans', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const { dryRun = true } = req.body;

        // Get all valid user IDs
        const validUserIds = (await User.find().select('_id')).map(u => (u as any)._id.toString());
...
// @access  Private (SuperAdmin)
router.post('/db/recalculate-all-balances', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const { dryRun = true } = req.body;
        ...
        for (const user of users) {
            ...
            for (const balanceType of ['breakfast', 'lunch', 'dinner']) {
                ...
                calculatedBalances[balanceType] = transactions.reduce((sum: number, t: any) => sum + t.amount, 0);
            }
            ...
            if (hasDifference) {
                ...
                if (!dryRun) {
                    (user as any).balances.breakfast.amount = calculatedBalances.breakfast;
                    (user as any).balances.lunch.amount = calculatedBalances.lunch;
                    (user as any).balances.dinner.amount = calculatedBalances.dinner;
                    await user.save();
                }
            }
        }
...
router.put('/meals/:id/force-edit', protect, isSuperAdmin, [
    ...
    try {
        ...
        const meal = await Meal.findById(req.params.id).populate('user', 'name email') as any;
        if (!meal) {
        ...
        meal.lastModifiedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.delete('/meals/:id/force-delete', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const { reason } = req.body;
        if (!reason) {
...
router.put('/holidays/:id/force-edit', protect, isSuperAdmin, [
    ...
    try {
        ...
        const holiday = await Holiday.findById(req.params.id) as any;
        if (!holiday) {
        ...
        holiday.modifiedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.delete('/holidays/:id/force-delete', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const { reason } = req.body;
        if (!reason) {
            return res.status(400).json({ message: 'কারণ আবশ্যক' });
...
router.put('/groups/:id/force-edit', protect, isSuperAdmin, [
    ...
    try {
        ...
        const group = await Group.findById(req.params.id).populate('manager', 'name').populate('members', 'name') as any;
        if (!group) {
        ...
        group.modifiedBy = (req.user as any)._id;
...
// @access  Private (SuperAdmin)
router.post('/users/:id/recalculate-balance', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        ...
        for (const type of balanceTypes) {
            ...
            const calculatedBalance = transactions.reduce((sum: number, t: any) => sum + t.amount, 0);
...
// @access  Private (SuperAdmin)
router.post('/recalculate-meal-costs', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const monthSettings = await MonthSettings.findOne({ year, month }) as any;
        if (!monthSettings) {
            return res.status(404).json({ message: 'মাসের সেটিংস পাওয়া যায়নি' });
        }
        ...
        const results: any = {
            ...
            lunchMeals: meals.filter((m: any) => m.mealType === 'lunch').length,
            dinnerMeals: meals.filter((m: any) => m.mealType === 'dinner').length,
        ...
        const userMealCounts: any = {};
        for (const meal of meals) {
            const userId = (meal as any).user._id.toString();
            if (!userMealCounts[userId]) {
                userMealCounts[userId] = {
                    userName: (meal as any).user.name,
                    lunch: 0,
                    dinner: 0
                };
            }
            if ((meal as any).mealType === 'lunch') {
                userMealCounts[userId].lunch += (meal as any).count;
            } else {
                userMealCounts[userId].dinner += (meal as any).count;
            }
        }

        for (const [userId, counts] of Object.entries(userMealCounts)) {
            const typedCounts = counts as any;
            const expectedLunchCost = typedCounts.lunch * monthSettings.lunchRate;
            const expectedDinnerCost = typedCounts.dinner * (monthSettings.dinnerRate || 0);

            results.recalculations.push({
                userId,
                userName: typedCounts.userName,
                lunchCount: typedCounts.lunch,
                dinnerCount: typedCounts.dinner,
                expectedLunchCost,
                expectedDinnerCost,
...
// @access  Private (SuperAdmin)
router.post('/system-reset', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
...
// @access  Private (SuperAdmin)
router.get('/data-summary', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const summary = {
            meals: {
                total: await Meal.countDocuments(),
...
// @access  Private (SuperAdmin)
router.get('/maintenance', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const settings = await GlobalSettings.getSettings() as any;
        res.json({
...
// @access  Private (SuperAdmin)
router.put('/maintenance/enable', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
...
// @access  Private (SuperAdmin)
router.put('/maintenance/disable', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const settings = await GlobalSettings.getSettings() as any;
...
// @access  Private (SuperAdmin)
router.get('/rate-rules', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const settings = await GlobalSettings.getSettings() as any;
        res.json({
...
// @access  Private (SuperAdmin)
router.put('/rate-rules/toggle', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const settings = await GlobalSettings.getSettings() as any;
...
// @access  Private (SuperAdmin)
router.put('/rate-rules/:index', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
...
// @access  Private (SuperAdmin)
router.delete('/rate-rules/:index', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const index = parseInt(req.params.index);
...
// @access  Private (SuperAdmin)
router.post('/rate-rules/calculate', protect, isSuperAdmin, async (req: AuthRequest, res: Response) => {
    try {
