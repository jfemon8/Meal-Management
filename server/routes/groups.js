...
const canManageGroup = async (req, res, next) => {
    try {
    ...
    try {
        ...
        if (!['admin', 'superadmin'].includes(req.user.role)) {
            ...
            if (req.user.isGroupManager) {
                ...
                query = { _id: req.user.group };
            } else {
                return res.json([]); // No groups to show
            }
        }

        const groups = await Group.findWithMemberCount(query);
        res.json(groups);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'সার্ভার এরর' });
...
router.get('/:id', protect, async (req, res) => {
    try {
        ...
        const canAccess = ['admin', 'superadmin'].includes(req.user.role) ||
            ...
            (req.user.group && req.user.group.toString() === group._id.toString());
...
router.get('/:id/members', protect, canManageGroup, async (req, res) => {
    try {
...
router.post('/:id/members', protect, canManageGroup, [
    ...
    try {
        ...
        if (req.managedGroup && !group.settings.canManagerAddUsers) {
        ...
        if (user.group) {
            ...
            return res.status(400).json({
                message: `এই ইউজার ইতোমধ্যে "${currentGroup ? currentGroup.name : 'অন্য'}" গ্রুপে আছে`
            });
        }
...
// @access  Private (Admin+ or Group Manager with permission)
router.delete('/:id/members/:userId', protect, canManageGroup, async (req, res) => {
    try {
        const group = await Group.findById(req.params.id);
        if (!group) {
            return res.status(404).json({ message: 'গ্রুপ পাওয়া যায়নি' });
        ...
        if (req.managedGroup && !group.settings.canManagerRemoveUsers) {
...
// @access  Private (Admin+)
router.get('/available-users/list', protect, isAdmin, async (req, res) => {
    try {
